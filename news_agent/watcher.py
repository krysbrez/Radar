...
# Nemovitosti adapter lives in sources/sreality.py — auta stays inline for now
# because the proven Sauto API logic with strict matching was hardened here.
from sources.sreality import search_sreality
...
# Fingerprint buckets — coarse enough to catch re-listings, fine enough
# to avoid collapsing genuinely different cars.
KM_BUCKET_STEP    = 5_000   # km rounded down to nearest 5 000
PRICE_BUCKET_STEP = 10_000  # Kč rounded down to nearest 10 000

AREA_BUCKET_STEP = 5         # m² rounded down to nearest 5
PRICE_BUCKET_NEMOV = 100_000  # Kč rounded down to nearest 100k for nemovitosti
...
SEARCHERS = {
    "auta":        search_sauto,      # inline in this module (proven MVP)
    "nemovitosti": search_sreality,   # from sources/sreality.py
}
...
if __name__ == "__main__":
    main()# Email delivery is intentionally disabled until Resend sender domain is
# verified and production-ready. Set to True only when ready.
EMAIL_ENABLED = False
