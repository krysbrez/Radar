      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="inline-flex items-center gap-2 bg-primary/5 rounded-full px-3 py-1 mb-2">
            <span className="text-xs font-black text-white uppercase tracking-widest font-headline">{t("bitesize.badge")}</span>
          </div>
          <h2 className="text-2xl font-black text-white font-headline">{t("bitesize.title")}</h2>
        </div>
        <span className="text-xs text-white/55 hidden md:block">{t("bitesize.this_week")}</span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {LINKS.map((item, i) => (
          <Link
            key={i}
            to={item.path}
            className="group bg-white rounded-2xl border border-white/12/10 p-5 hover:shadow-md hover:border-white/12/25 transition-all flex flex-col"
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-white/55">{item.readTime} {t("bitesize.read_min")}</span>
            </div>
            <div className="text-2xl mb-2">{item.emoji}</div>
            <p className="font-black text-white font-headline text-sm leading-tight mb-2 group-hover:text-white-container transition-colors flex-1">{item.title}</p>
            <p className="text-xs text-white/65 leading-relaxed">{item.desc}</p>
            <div className="mt-3 flex items-center gap-1 text-xs font-bold text-white group-hover:gap-2 transition-all">
