  ...
  return (
    <section className="max-w-7xl mx-auto px-6 md:px-8 py-16">
      <div className="flex items-end justify-between mb-10">
        <div>
          <p className="text-xs font-black tracking-widest uppercase text-outline mb-2 font-headline">Real talk</p>
          <h2 className="text-4xl md:text-5xl font-black text-primary font-headline tracking-tight leading-none">
            Příběhy ze života
          </h2>
        </div>
        <p className="hidden md:block text-sm text-on-surface-variant max-w-xs text-right">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {STORIES.map((s) => (
          <div
            key={s.name}
            className="bg-white rounded-2xl border border-outline-variant/10 p-6 hover:shadow-md hover:border-outline-variant/25 transition-all flex flex-col"
          >
            <div className="flex items-center gap-3 mb-4">
              <img
                ...
                onError={(e) => { e.target.src = ""; e.target.className = "w-12 h-12 rounded-full bg-surface-container flex-shrink-0"; }}
              />
              <div>
                <p className="font-black text-primary font-headline">{s.name}</p>
                <p className="text-xs text-outline">{s.age} let · <span className={`font-bold px-1.5 py-0.5 rounded-full text-xs ${s.tagColor}`}>{s.tag}</span></p>
              </div>
            </div>

            <p className="text-sm text-on-surface leading-relaxed flex-1 mb-5 italic">
              "{s.story}"
            </p>
