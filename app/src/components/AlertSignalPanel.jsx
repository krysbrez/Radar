...
export default function AlertSignalPanel({
  title = "Radar Alerts",
  subtitle = "Ukázka alertů napříč hlavními segmenty",
  ...
  return (
    <div className="mt-5 rounded-[1.5rem] border border-white/10 bg-white/8 backdrop-blur-sm p-4">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div>
          <p className="text-white text-sm font-black font-headline tracking-tight">{title}</p>
          <p className="text-primary-fixed-dim text-xs">{subtitle}</p>
        </div>
        <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-primary-fixed-dim">
      ...
      <div className="space-y-2">
        {alerts.map((alert) => (
          <div
            ...
            className="flex items-start gap-3 rounded-xl bg-black/15 px-3 py-2.5"
          >
            <span className={`mt-1 h-2 w-2 flex-shrink-0 rounded-full ${toneClass[alert.tone] ?? toneClass.neutral}`} />
            <div className="min-w-0 flex-1">
              <p className="mb-1.5 text-[11px] font-black uppercase leading-tight tracking-[0.18em] text-primary-fixed-dim">
                {alert.label}
              </p>
