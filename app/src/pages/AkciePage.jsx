  ...
  return (
    <div className="bg-white/5 rounded-2xl border border-white/8 overflow-hidden">
      <div className="px-5 py-4 border-b border-white/8 flex items-center gap-2">
        ...
        <h3 className="font-black text-white font-headline text-base">{title}</h3>
      </div>
      {/* Desktop */}
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {stocks.map((s, i) => (
              <tr key={s.symbol} className={`border-t border-white/6 hover:bg-white/5 transition-colors ${i % 2 === 1 ? "bg-white/3" : ""}`}>
                <td className="px-5 py-3.5 font-bold text-white">{s.name}</td>
                <td className="px-4 py-3.5">
                  <span className="bg-white/5 px-2 py-0.5 rounded font-mono text-xs text-white/55 font-bold">{s.symbol}</span>
                </td>
                <td className="px-4 py-3.5 text-white/65 text-xs">{s.sector}</td>
                <td className="px-4 py-3.5 text-right font-bold text-white font-mono">{s.price}</td>
                <td className="px-4 py-3.5">
                ...
                <td className="px-5 py-3.5 text-right">
                  <span className={`font-black text-sm px-2 py-0.5 rounded-full ${s.isUp ? "text-emerald-400 bg-emerald-500/15" : "text-red-400 bg-red-500/15"}`}>{s.change}</span>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        {stocks.map((s) => (
          <div key={s.symbol} className="px-4 py-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-white/55 font-mono">{s.symbol} · {s.sector}</p>
            </div>
            <div className="text-right">
              <p className="font-bold text-white font-mono text-sm">{s.price}</p>
              <span className={`text-xs font-black px-2 py-0.5 rounded-full ${s.isUp ? "text-emerald-400 bg-emerald-500/15" : "text-red-400 bg-red-500/15"}`}>{s.change}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    <Link to={`/clanek/${article.id}`} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-white/8 bg-white/5 transition-shadow hover:border-white/15 hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="mb-3 flex min-h-6 flex-wrap items-center gap-2">
          <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full ${article.tagColor || "bg-indigo-500/15 text-indigo-300"}`}>{article.tag}</span>
      ...
      <div className="gradient-primary text-white">
        <div className="max-w-5xl mx-auto px-6 md:px-8 pt-10 pb-12">
          <nav className="flex items-center gap-2 text-xs text-white/50 mb-6">
            ...
            <span>›</span>
            <span className="text-white/75">{t("akcie.breadcrumb")}</span>
          </nav>
          <div className="flex flex-col md:flex-row items-center gap-10">
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 bg-white/10 rounded-full px-4 py-1.5 mb-4">
                <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                <span className="text-xs font-black text-white/75 uppercase tracking-widest font-headline">Live data · 5. 4. 2026</span>
              </div>
              <h1 className="text-4xl md:text-5xl font-black font-headline tracking-tight mb-4">{t("akcie.hero_title")}<br /><span className="text-[#ffd700]">{t("akcie.hero_title_accent")}</span></h1>
              <p className="text-white/75 text-lg leading-relaxed max-w-xl">{t("akcie.hero_subtitle")}</p>
              <div className="flex flex-wrap gap-3 mt-6">
                <a href="#tabulka" className="bg-white text-[#000613] font-black text-sm font-headline px-5 py-2.5 rounded-full hover:bg-white/90 transition-colors">
                  {t("akcie.btn_table")}
                </a>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-8 py-10 space-y-12">
        ...
        <section>
          <div className="grid md:grid-cols-2 gap-6 items-center">
            <div>
              <div className="inline-flex items-center gap-2 bg-indigo-500/10 rounded-full px-3 py-1 mb-3">
                <span className="text-xs font-black text-indigo-300 uppercase tracking-wider font-headline">Základy</span>
              </div>
              <h2 className="text-2xl font-black font-headline text-white mb-4">Co je vlastně akcie?</h2>
              <p className="text-white/85 leading-relaxed mb-3">Firma potřebuje peníze na růst. Místo půjčky od banky vydá <strong>akcie</strong> — malé kousky vlastnictví. Ty je koupíš a staneš se spolumajitelem.</p>
              <p className="text-white/85 leading-relaxed mb-3">Pokud firma vydělává, roste i hodnota akcií. Některé firmy navíc vyplácejí <strong>dividendy</strong> — pravidelné platby akcionářům.</p>
              <p className="text-white/85 leading-relaxed">Riziko? Firma může i prodělat. Proto se vyplatí investovat do víc firem (nebo ETF), ne jen do jedné.</p>
            </div>
            <div className="bg-indigo-500/8 border border-indigo-500/20 rounded-2xl p-6">
              <p className="text-xs font-black text-indigo-300 uppercase tracking-wider font-headline mb-3">Příklad z reálného života</p>
        ...
        <section id="tabulka" className="space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-black font-headline text-white mb-2">Přehled akcií</h2>
            <p className="text-white/65 text-sm">{t("akcie.data_note")}</p>
          </div>
          ...
          <div className="bg-[#ffd700]/8 border border-[#ffd700]/20 rounded-xl p-4 flex gap-2">
            <span className="text-base flex-shrink-0">⚠️</span>
            <p className="text-sm text-[#ffd700]/80 leading-relaxed">{t("akcie.disclaimer")}</p>
          </div>
        </section>
        ...
        <section id="jak-koupit">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-black font-headline text-white mb-2">Jak koupit první akcii</h2>
            <p className="text-white/65 text-sm">{t("akcie.steps_subtitle")}</p>
          </div>
          <div className="grid md:grid-cols-3 gap-4">
            {HOW_TO_BUY.map((step) => (
              <div key={step.number} className="bg-white/5 border border-white/8 rounded-2xl p-5">
                <div className={`w-10 h-10 ${step.color} rounded-xl flex items-center justify-center text-lg mb-4`}>{step.emoji}</div>
                <div className={`text-xs font-black uppercase tracking-wider font-headline ${step.stepColor} mb-1`}>Krok {step.number}</div>
                <h3 className="font-black text-white font-headline text-base mb-2">{step.title}</h3>
                <p className="text-sm text-white/85 leading-relaxed mb-3">{step.desc}</p>
                <div className="bg-white/8 rounded-xl px-3 py-2">
                  <p className="text-xs text-white/65">💡 {step.tip}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="grid md:grid-cols-3 gap-6">
          <div className="md:col-span-2 bg-white/5 rounded-2xl border border-white/8 p-6">
            <h3 className="font-black text-white font-headline text-lg mb-4">{t("akcie.vs_etf_title")}</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="bg-indigo-500/8 border border-indigo-500/20 rounded-xl p-4">
                <p className="font-black text-indigo-300 font-headline mb-2">📊 Jednotlivé akcie</p>
                <ul className="space-y-1.5 text-white/85">
                  <li className="flex gap-2"><span className="text-emerald-400">✓</span> Vyšší potenciál výnosu</li>
                  <li className="flex gap-2"><span className="text-emerald-400">✓</span> Vlastníš konkrétní firmu</li>
              ...
              <div className="bg-purple-500/8 border border-purple-500/20 rounded-xl p-4">
                <p className="font-black text-purple-300 font-headline mb-2">🧺 ETF (košík akcií)</p>
                <ul className="space-y-1.5 text-white/85">
                  <li className="flex gap-2"><span className="text-emerald-400">✓</span> Automatická diverzifikace</li>
                  <li className="flex gap-2"><span className="text-emerald-400">✓</span> Nízké poplatky</li>
                  <li className="flex gap-2"><span className="text-emerald-400">✓</span> Méně práce</li>
                  <li className="flex gap-2"><span className="text-red-400">✗</span> Průměrný výnos trhu</li>
                </ul>
              </div>
            </div>
            <p className="text-sm text-white/65 mt-4">Pokud začínáš, dává největší smysl <strong className="text-white">začít s ETF</strong>. Jednotlivé akcie si nech na chvíli, kdy už víš, co vlastně kupuješ.</p>
          </div>
        ...
        {articles.length > 0 && (
          <section>
            {starterArticle && (
              <div className="mb-5 rounded-2xl border border-indigo-500/20 bg-indigo-500/8 p-5">
                <p className="text-[11px] font-black uppercase tracking-[0.2em] text-indigo-300 font-headline">
                  Čti nejdřív
                </p>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <Link
                    ...
                    className="rounded-full bg-indigo-600 px-4 py-2 text-sm font-black text-white font-headline hover:bg-indigo-500 transition-colors"
                  >
                  ...
                  {nextArticle && (
                    <Link
                      ...
                      className="rounded-full border border-indigo-500/30 bg-white/5 px-4 py-2 text-sm font-bold text-indigo-300 font-headline hover:border-indigo-500/50 hover:bg-indigo-500/10 transition-colors"
                    >
            ...
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-black font-headline text-white">{t("akcie.articles_section_title")}</h2>
              <Link to="/archiv" className="text-sm font-bold text-white/55 hover:text-white transition-colors">Všechny články →</Link>
            </div>
        ...
        <div className="gradient-primary rounded-2xl p-8 text-white text-center">
          ...
          <p className="text-white/75 mb-6 max-w-md mx-auto">Každý týden přinášíme přehled trhu, analýzy a tipy pro začátečníky i pokročilé.</p>
          <div className="flex flex-wrap justify-center gap-3">
            <Link to="/knowhow" className="bg-white text-[#000613] font-black text-sm font-headline px-6 py-2.5 rounded-full hover:bg-white/90 transition-colors">
              Know How sekce →
            </Link>
