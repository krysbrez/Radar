    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="gradient-primary text-white">
        <div className="max-w-5xl mx-auto px-6 md:px-8 pt-10 pb-12">
          <nav className="flex items-center gap-2 text-xs text-primary-fixed-dim/60 mb-6">
            ...
            <span>›</span>
            <span className="text-primary-fixed-dim">IPO</span>
          </nav>
          <div className="flex flex-col md:flex-row items-center gap-10">
            <div className="flex-1">
              <div className="inline-flex items-center gap-2 bg-white/10 rounded-full px-4 py-1.5 mb-4">
                <span className="text-xs font-black text-primary-fixed-dim uppercase tracking-widest font-headline">{t("ipo.hero_badge")}</span>
              </div>
              ...
              <p className="text-primary-fixed-dim text-lg leading-relaxed max-w-xl">{t("ipo.hero_subtitle")}</p>
            </div>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-8 py-10 space-y-12">
        ...
        <section className="grid md:grid-cols-2 gap-6 items-center">
          <div>
            <h2 className="text-2xl font-black font-headline text-primary mb-4">{t("ipo.what_is")}</h2>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.t} className="bg-white rounded-xl border border-outline-variant/10 p-4 flex gap-3">
                <span className="text-xl flex-shrink-0">{item.icon}</span>
                <div>
                  <p className="font-bold text-primary text-sm">{item.t}</p>
                  <p className="text-xs text-on-surface-variant">{item.d}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Nadcházející IPO */}
        <section>
          <h2 className="text-xl font-black font-headline text-primary mb-5">{t("ipo.upcoming")}</h2>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {UPCOMING_IPOS.map((ipo, i) => (
                    <tr key={ipo.company} className={`border-t border-outline-variant/5 ${i % 2 === 1 ? "bg-surface-container/20" : ""}`}>
                      <td className="px-5 py-3.5 font-bold text-primary">{ipo.flag} {ipo.company}</td>
                      <td className="px-4 py-3.5 text-on-surface-variant text-xs">{ipo.sector}</td>
            ...
            <div className="md:hidden divide-y divide-outline-variant/5">
              {UPCOMING_IPOS.map(ipo => (
                <div key={ipo.company} className="px-4 py-3 flex justify-between items-center">
                  <div>
                    <p className="font-bold text-primary text-sm">{ipo.flag} {ipo.company}</p>
                    <p className="text-xs text-outline">{ipo.sector} · {ipo.date}</p>
                  </div>
          ...
          <p className="text-xs text-outline mt-2">{t("ipo.disclaimer")}</p>
        </section>

        {/* Nedávné IPO */}
        <section>
          <h2 className="text-xl font-black font-headline text-primary mb-5">{t("ipo.recent")}</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {RECENT_IPOS.map(ipo => (
              <div key={ipo.company} className="bg-white rounded-2xl border border-outline-variant/10 p-4 text-center">
                <p className="text-lg mb-1">{ipo.flag}</p>
                <p className="font-black text-primary font-headline text-sm">{ipo.company}</p>
                <p className="text-xs text-outline mb-2">{ipo.date}</p>
                <span className={`text-lg font-black font-headline ${ipo.isUp ? "text-green-600" : "text-red-500"}`}>{ipo.change}</span>
                <p className="text-xs text-outline mt-1">{t("ipo.since_ipo")}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Jak koupit IPO */}
        <section>
          <h2 className="text-xl font-black font-headline text-primary mb-5">{t("ipo.how_to")}</h2>
          <div className="grid md:grid-cols-2 gap-4">
            {HOW_TO.map(step => (
              <div key={step.n} className="bg-white rounded-2xl border border-outline-variant/10 p-5 flex gap-4">
                ...
                <div>
                  <p className="font-bold text-primary font-headline">{step.t}</p>
                  <p className="text-sm text-on-surface-variant mt-1 leading-relaxed">{step.d}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="gradient-primary rounded-2xl p-8 text-white text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-4">
            <Link to="/akcie" className="bg-white text-primary font-black text-sm font-headline px-6 py-2.5 rounded-full hover:bg-primary-fixed transition-colors">
              {t("ipo.cta_stocks")}
            </Link>
